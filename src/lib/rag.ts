...
export function dedupeSupplementText(
  ...
  if (kept.length === 0) {
    return (
      ...
      "가이드라인 답변을 바탕으로, 일상에서 바로 적용할 수 있는 구체적 방법을 더 정리하려면 증상·상황을 조금 더 알려주세요."
    );
  }
...
가이드라인 답변을 받은 뒤 보충 설명을 요청했습니다. 아래 '이미 답변한 내용'과 겹치지 않게, 실생활에 쓸 수 있는 내용을 3~4개 항목으로 짧게 이어서 쓰세요. 진료·병원 방문과 연결하거나 의료진에게 넘기는 표현은 쓰지 마세요.
