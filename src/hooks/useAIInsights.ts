...
export function useAIInsights() {
  ...
  const generate = async (profile: PatientProfile) => {
    ...
    const prompt = `당신은 "화순전남대학교병원" 소속 폐암 전문의입니다. ${age}세 ${gender === 'female' ? '여성' : '남성'} 환자(${histology}, 변이: ${mutationLabels}, PD-L1: ${pdl1Label}, 치료 실시: ${isTreated ? '예' : '아니오'})를 위한 4기 폐암 관리 정밀 리포트를 따뜻하게 작성해주세요. 최신 치료법을 언급하고 마지막엔 '화순전남대학교병원 폐암 전문의 드림'이라고 적어주세요.`;
