import { EMBEDDING_MODEL, TEXT_MODEL, isOpenAIConfigured } from 'file:///Users/nks/Documents/LCA-hwa/src/constants';
import {
  callOpenAIEmbed,
  callOpenAIChat,
  type OpenAIChatMessage,
} from 'file:///Users/nks/Documents/LCA-hwa/src/lib/openai';
import { histologyLabel } from 'file:///Users/nks/Documents/LCA-hwa/src/lib/utils';
import type {
  GuideAnswerType,
  GuideChatMessage,
  GuideChatSource,
  GuideSearchMode,
  Histology,
} from 'file:///Users/nks/Documents/LCA-hwa/src/types';
...
const SUPPORTIVE_CARE_QUERY =
  /부작용|지지|완화|증상|supportive|palliative|side effect|피로|오심/i;
...
const GUIDELINE_FOLLOWUP_QUERY =
  /가이드라인.*(없|있|은|이|에|에서|에선)|뭐래|뭐라|관련\s*내용|원문.*(없|있)|^가이드라인\s*\??$/i;
...
function isNavigationOrTocText(text: string): boolean {
  ...
  const pageRefs =
    clean.match(
      /\b\d{1,3}\s+(?:Types of|Planning|Staging|Treatment|Early|Stage|Key points|Questions|Improving)/gi,
    ) ?? [];
...
function excerptRelatesToQuery(excerpt: string, query: string): boolean {
  ...
  if (NUTRITION_QUERY.test(query)) {
    return /healthful|healthy living|body weight|diet|nutrition/i.test(exLower);
  }
  if (TREATMENT_METHOD_QUERY.test(query)) {
    return /surgery|surgical|lobectomy|radiation|chemotherapy|chemoradiation|treatment/i.test(
      exLower,
    );
  }
  if (DAILY_LIVING_QUERY.test(query) || SUPPORTIVE_CARE_QUERY.test(query)) {
    return /supportive care|quality of life|fatigue|infection|hygiene|exercise|nutrition|emotional|rest|symptom|palliative|nausea|vomiting|antiemetic|side effect|complication/i.test(
      exLower,
    );
  }
  if (SUN_UV_QUERY.test(query)) {
...
export function filterRelevantCitations(
  ...
  return citations.filter((c) => excerptRelatesToQuery(c.excerpt, query));
}
...
export async function retrieveChunks(
  ...
  const ranked = hasEmbeddings(store)
    ...
    : pool
        .map((chunk) => {
          const similarity = keywordScore(query, chunk.text) / 10;
          return {
            chunk,
            similarity,
            score: similarity + docBoost(targetDocs, chunk.docId),
...
- 질문의 핵심에 맞게 일반 환자 교육 수준으로 **충분히** 설명하세요.
...
**답변 규칙**
- **첫 문장**에 사용자 질문에 대한 핵심 답을 말하세요. (후속 질문이면 "가이드라인에는 ~가 안내되어 있습니다" / "직접 다루지 않습니다" 등을 명확히)
- 아래 [참고 원문]에 **실제로 있는 내용만** 근거로 설명하세요. 원문과 무관한 일반론만 길게 쓰지 마세요.
...
function citationsAreTrustworthy(
  citations: GuideChatSource[],
  query: string,
  relevanceScore: number,
): boolean {
  const relevant = filterRelevantCitations(citations, query);
  if (relevant.length === 0) return false;

  if (hasTopicAnchor(query)) return true;

  if (relevant.length >= 2 && relevanceScore >= MIN_CITATION_SIMILARITY) {
    return true;
  }

  return relevanceScore >= 0.57;
}
